...
description: "Learn how to authenticate with the Pet Store API using API keys and bearer tokens."
---

# Authentication

The Pet Store API uses multiple authentication methods to ensure secure access to your data. Choose the method that best fits your use case and security requirements.

## Authentication Methods

### API Key Authentication

The simplest way to authenticate with the Pet Store API is using an API key passed in the request header.
...
```bash
curl -X POST "https://api.petstore.example.com/v1/auth/login" \
  -H "Content-Type: application/json" \
  -d '{
    "username": "john.doe",
...
```javascript
const loginResponse = await fetch('https://api.petstore.example.com/v1/auth/login', {
  method: 'POST',
  headers: {
    'Content-Type': 'application/json'
  },
...
response = requests.post(
    'https://api.petstore.example.com/v1/auth/login',
    json=login_data
)
...
class PetStoreAPI {
  ...
  async authenticate() {
    const response = await fetch('https://api.petstore.example.com/v1/auth/login', {
      method: 'POST',
  ...
  async apiRequest(endpoint, options = {}) {
    ...
    const response = await fetch(`https://api.petstore.example.com/v1${endpoint}`, {
      ...options,
      headers: {
    ...
    if (response.status === 401) {
      ...
      return fetch(`https://api.petstore.example.com/v1${endpoint}`, {
        ...options,
        headers: {
...
1. **Log in to your Pet Store dashboard** at [dashboard.petstore.example.com](https://dashboard.petstore.example.com)
...
PETSTORE_BASE_URL=https://api.petstore.example.com/v1
```
...
<Callout kind="success">
  **Authentication setup complete!** You're now ready to start making authenticated requests to the Pet Store API. Check out our [Getting Started guide](getting-started) for your first API calls.
</Callout>
...
- **[Pet Store API Reference](/api-reference/petstore-openapi.yaml)** - Explore all available endpoints
